...
export const Boundbox=styled.div`
     border-radius: 10px;
     background:${(props) => (props.variant == 1 ? ezGreen : props.variant == 2? ezYellow: props.variant == 3? ezRed:ezGreen)};
...
var returnTime=(timeStart,timeEnd)=>{
    ...
    return hour1+":"+ minute1+" - "+ hour2+":"+ minute2+",  "+ day1+"/"+month1+"/"+year1;
}
        ...
        <Bigbox className='example'>
          {
              requestList.map((request)=>(
                  <FlatList onClick={() => showPopUp(true,request)}>
                      <div style={{margin:'0.5rem'}}><RoomAvatar roomId={request.roomId}></RoomAvatar></div>
                     <div style={{display:'flex',flexDirection:'column',flex:'4',margin:'1rem'}}>
                         <Typography textAlign={'start'} color={'#808080'}>{returnTime(request.startTime,request.endTime)}</Typography>
                         <Typography textAlign={'start'}>{request.description}</Typography>
                     </div>
                     <Boundbox variant={request.status}><Typography variant='h5'margin={'0.5rem'} color={request.status==1? "#fff": request.status==2 ?"#000" : request.status==3 ?"#fff": "#fff"} >
                         {request.status==1? "Approved": request.status==2 ?"Pending" : request.status==3 ?"Decline": "Approved"}</Typography></Boundbox>
                  </FlatList>
              ))
          }
